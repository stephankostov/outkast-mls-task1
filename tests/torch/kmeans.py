...
from mls_1.torch.kmeans import *
...
def test_kmeans(datapoints, k, distance_metric, tol):
    ...
    (cp_centroids, cp_labels), time = time_function(kmeans, 
        torch.tensor(datapoints), k=k, distance_metric=distance_metric, tol=tol, initialized_centroids=torch.tensor(initialized_centroids))

    numpy_centroids_sorted = np.sort(numpy_centroids, axis=0)
    numpy_centroids_vec_sorted = np.sort(numpy_centroids_vec, axis=0)
    cp_centroids_sorted = np.sort(torch.tensor(cp_centroids), axis=0)
